...
export class HardwareBackButtonService {
  ...
  constructor(private _plt: Platform, private _menuCtrl: MenuController,
              private _toastCtrl: ToastController) { }

  registerAction(fn) {
    this._deregisterFn = this._plt.registerBackButtonAction(() => fn());
  }
  ...
  doubleBackToExit() {
    ...
    if (!this._didBackAlready) {
      ...
      this.presentToast('Press back button again to exit');
  ...
  private presentToast(content: string) {
    const toast = this._toastCtrl.create({ message: content, position: 'bottom', duration: 2000 });
    toast.present();
  }
